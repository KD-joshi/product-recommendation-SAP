...
This repository contains my submission for the SAP CX II Technical Exercise. I have built a **Multimodal Product Similarity Search Engine** leveraging a Dense Vector Retrieval architecture (Sentence-BERT + CLIP Visual Embeddings + FAISS HNSW).
...
### The Core Function
The core similarity search algorithm requested in the assignment is located in `similarity_search.py`. It handles FAISS HNSW querying to provide similar products based on text, structured metadata, and images, comparable to mainstream e-commerce search engines like Amazon.
```python
from similarity_search import ProductSimilaritySearch

search = ProductSimilaritySearch()
search.load("indices")
similar_products = search.find_similar_products("3a92218968565a0b5a1900a6eefd309f", num_similar=10, mode="text_structured")
```
...
**Core Endpoint:** `GET /find_similar_products`
...
Run the offline indexing pipeline. This script utilizes Polars for NDJSON parsing, runs the Sentence-BERT/CLIP neural embeddings, and constructs the FAISS HNSW graphs.
```bash
...
This interactive dashboard allows you to paste any `product_id`, adjust the `num_similar` slider, toggle between the different neural engines, and visually see the product recommendations side-by-side.

### Interactive UI Demos

**Image Only Search** — Discovering similar striped shirts using pure CLIP visual embeddings:
<br>
<img src="assets/demo_ui_image_search.png" width="800">

**Hybrid Search** — Fetching identical Arrow Sports casual shirts using Semantic Text and Structured Data:
<br>
...
| `IMAGE_MODEL_NAME` | `openai/clip-vit-base-patch32` | The model used for visual embeddings (generates 512-dimensional vectors). |
...
| `MAX_DOWNLOAD_THREADS` | `4` | Max parallel connections used to download images for CLIP processing. |

---

## 5. Evaluation & Benchmarking

I wrote an automated test suite to validate the speed and accuracy of the search engine:

**Latency Benchmarks**: Tests the Queries-Per-Second (QPS) of the different modes.
```bash
